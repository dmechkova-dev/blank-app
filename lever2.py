...
tab1, tab2, tab3 = st.tabs(["🏗️ Интерактивен Лост", "⚙️ Макари", "📝 Тест за знания"])
...
with tab3:
    ...
    st.write("Отговори на всички въпроси и натисни бутона най-отдолу.")
    ...
    if st.button("🏁 Предай теста"):
        # Проверка
        if q1 == "Променяме посоката на силата за удобство": score += 1
        if q2 == "2 пъти по-малка от F2": score += 1
        if q3 == "2 пъти": score += 1
        if q4 == 50: score += 1
        if q5 == "Губим път (дърпаме двойно повече въже)": score += 1
        if q6 == "Прости механизми": score += 1
        
        # Резултат
        st.subheader(f"Твоят резултат: {score} от 6")
        
        if score == 6:
            st.balloons()
            st.success("Отличен 6! Ти познаваш механизмите перфектно! 🏆")
        elif score >= 4:
            st.warning("Много добър! Имаш малки пропуски, прегледай симулациите пак. 👍")
        else:
            st.error("Трябва да упражниш още малко. Провери формулите в първите два раздела! 📚")
